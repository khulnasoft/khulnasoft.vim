...
local glab = require('khulnasoft.glab')
local curl = require('khulnasoft.curl')
local rest = require('khulnasoft.api.rest')
local config = require('khulnasoft.config')

describe('khulnasoft.api.rest', function()
  local curl_mock
  local glab_mock
  ...
  describe('api_v4_url', function()
    it('generates an API 4 URL from a path', function()
      ...
      assert.equal('https://khulnasoft.example.com/api/v4/projects/123', result)
    ...
    it('handles khulnasoft_url with relative url ending in slash', function()
      local config_mock = mock(config, true)
      config_mock.current.returns({ khulnasoft_url = 'https://khulnasoft.example.com/my/relative-url/' })

      local result = rest.api_v4_url('projects/123')
      assert.equal('https://khulnasoft.example.com/my/relative-url/api/v4/projects/123', result)
