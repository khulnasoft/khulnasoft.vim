...
guard_describe('khulnasoft.commands.code_suggestions', conditions.snowplow_micro_available, function()
  ...
  guard_describe(
    ...
    conditions.token({ using = 'GITHUB_TOKEN', require_scopes = { 'api' } }),
    function()
      before_each(function()
        local status, err = pcall(function()
          local env_auth = mock(require('khulnasoft.authentication.provider.env'))
          stub(env_auth, 'resolve').returns(env_auth)
          stub(env_auth, 'token').returns(vim.env.GITHUB_TOKEN)
          stub(env_auth, 'token_set').returns(true)
          stub(env_auth, 'url').returns('https://khulnasoft.com')
      ...
      it('opens the popup menu', function()
        ...
        vim.cmd.KhulnaSoftCodeSuggestionsStart()
