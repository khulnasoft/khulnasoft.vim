...
import (
	...
	"github.com/khulnasoft/khulnasoft/pkg/commands"
	"github.com/khulnasoft/khulnasoft/pkg/tcp"
	"github.com/khulnasoft/khulnasoft/pkg/testies"
	"github.com/khulnasoft/khulnasoft/pkg/window"
)

func render(win *window.SimpleAsciiWindow) {
	bytes, err := os.ReadFile("lua/khulnasoft/integration/khulnasoft")
	str := string(bytes)
...
func main() {
	...
	for {
		...
		case commands.PARTIAL_RENDER:
			...
			partialRender(&win, row, col, []byte("khulnasoft"))
