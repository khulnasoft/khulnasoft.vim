...
type RunParams struct {
	One        string `json:"one"`        // ai:anthropic:claude-3-7-sonnet-latest:prompt/KHULNASOFT
	Two        string `json:"two"`        // ai:openai:model:prompt/KHULNASOFT
		...
		for range params.count {
			for aIdx := range len(params.a) {
				for bIdx := range len(params.b) {
					round++
					one := fmt.Sprintf("ai:anthropic:%s:prompt/KHULNASOFT", params.a[aIdx])
					two := fmt.Sprintf("ai:openai:%s:prompt/KHULNASOFT", params.b[bIdx])
...
func main() {
	...
	params := RunParams{
		One: "ai:anthropic:claude-3-7-sonnet-latest:prompt/KHULNASOFT",
		Two: "ai:openai:gpt-4o-mini:prompt/KHULNASOFT",
		RoundTime: 30,
		Seed: 42069,
