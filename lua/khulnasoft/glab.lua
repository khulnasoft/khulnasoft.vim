...
local function _default_opts()
    local auth =
        require("khulnasoft.authentication").default_resolver():resolve()
    if auth then
        return {
            env = {
                GITHUB_TOKEN = auth.token(),
                KHULNASOFT_URI = auth.url(),
            },
        }
    end
end

-- Lua module: khulnasoft.glab
--
-- Exposes KhulnaSoft CLI commands.
local M = { name = "glab" }
...
function M.auth_status()
    ...
    local job, err =
        jobs.start_wait({ "glab", "auth", "status" }, _default_opts())
    if err then
        return nil, err
    end
...
function M.enabled()
    return not vim.g.khulnasoft_api_provider_glab_disabled
end
