local npm = require("khulnasoft.lib.npm")

-- Lua module: khulnasoft.health
local M = {}

local function _check_node_version(cmd) --{{{
    local node = cmd[1]
    local job, err = require("khulnasoft.lib.jobs").start_wait(
        { node, "--version" },
        {}
    )
    if err then
...
local function _check_npm_ls() --{{{
    ...
    if job then
        ...
        local advice = {
            string.format(
                "Install the KhulnaSoft Language Server via package registry:\n\t\tcd %s\n\t\t%s",
                npm_install.cwd,
                vim.fn.join(npm_install.cmd)
            ),
            "For more information review https://github.com/khulnasoft/khulnasoft.vim#troubleshooting",
        }
        if not package then
        ...
        if package.version and has_problems then
            vim.health.warn(
                string.format(
                    "@gitlab-org/gitlab-lsp installed (v%s)",
                    package.version
                ),
                advice
            )
...
M.check = function()
    local lsp_server = require("khulnasoft.lsp.server").new()
    ...
    if lsp_server:is_node() then
        vim.health.ok("Using khulnasoft-lsp package")
        ...
        vim.health.warn("Using a user-defined language server executable", {
            "Prefer the khulnasoft-lsp npm package where possible.",
        })
    end
end
