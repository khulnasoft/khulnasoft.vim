...
local function resolve_node_main_script()
    local lsp_package_dir = utils.joinpath(
        require("khulnasoft").plugin_root(),
        "node_modules",
        "@khulnasoft",
        "khulnasoft-lsp"
    )
    ...
    local package_json =
        io.open(utils.joinpath(lsp_package_dir, "package.json"), "r")
    ...
    return utils.joinpath(
        lsp_package_dir,
        vim.json.decode(json)["bin"]["khulnasoft-lsp"]
    )
end
...
function M.new()
    ...
    return {
        ...
        is_installed = function(self)
            ...
            if self.is_node() then
                return node_main_script ~= ""
                    and vim.loop.fs_stat(node_main_script)
            end
