local memoize = require("khulnasoft.lib.memoize")
local rest = require("khulnasoft.api.rest")

-- Lua module: khulnasoft.authentication.personal_access_token
local M = {}
...
function M.from_env(name)
    ...
    error(
        string.format(
            'Missing personal access token in environment variable "%s"',
            name
        )
    )
end
