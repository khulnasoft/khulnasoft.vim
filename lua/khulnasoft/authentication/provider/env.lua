...
function M.new(options)
    ...
    vim.validate({
        ["options.defaults"] = {
            options.defaults,
            validate.is_dict_of("string"),
        },
        ["options.keys"] = { options.keys, validate.is_dict_of("string") },
    })

    vim.validate({
        ["options.defaults.khulnasoft_url"] = {
            options.defaults.khulnasoft_url,
            "string",
        },
        ["options.keys.khulnasoft_url"] = {
            options.keys.khulnasoft_url,
            "string",
        },
    ...
    local env = {
        ...
        url = function()
            return vim.env[options.keys.khulnasoft_url]
                or options.defaults.khulnasoft_url
        end,
    }
    setmetatable(env, {
        __index = M.prototype,
