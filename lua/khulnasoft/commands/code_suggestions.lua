...
function CodeSuggestionsCommands:install_language_server()
    ...
    notifier.notify(
        "khulnasoft.vim: Installing @gitlab-org/gitlab-lsp under "
            .. lsp_path
            .. "",
...
function CodeSuggestionsCommands:stop()
    if self.lsp_client then
        ...
        notifier.notify(
            "khulnasoft.vim: No active client found.",
            vim.lsp.log_levels.DEBUG
        )
    end
    ...
    create = function(options)
        ...
        vim.api.nvim_create_autocmd({ "CompleteDonePre" }, {
            callback = function()
                ...
                if
                    config.language_server.workspace_settings.telemetry.enabled
                then
                    ...
                    local suggestion =
                        suggestion_data_from_completion_item(items[1])
            ...
            callback = function()
                ...
                then
                    ...
                    if completed_item then
                        local suggestion =
                            suggestion_data_from_completion_item(completed_item)
                        if suggestion and suggestion.trackingId then
                            suggestion_events[suggestion.trackingId] = {
                    ...
                    for trackingId, event in pairs(suggestion_events) do
                        ...
                        code_suggestions_commands.lsp_client:notify(
        ...
        vim.api.nvim_create_user_command(
            "KhulnaSoftCodeSuggestionsInstallLanguageServer",
            function()
        ...
        vim.api.nvim_create_user_command(
            "KhulnaSoftCodeSuggestionsStart",
            function()
        ...
        vim.keymap.set(
            "n",
            "<Plug>(KhulnaSoftToggleCodeSuggestions)",
            function()
