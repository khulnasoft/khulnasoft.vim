...
function M.resource_url_to_api_url(url)
    ...
    local base_url = cfg.khulnasoft_url:gsub("/$", "")
    local resource_path = string_without_prefix(url, base_url)

    if not resource_path then
        return nil,
            url .. " does not start with khulnasoft_url " .. cfg.khulnasoft_url
    end

    local parent_path, resource_type, resource_id =
        resource_path:match("/(.+)%/%-%/([%w_]+)%/(%d+)")

    if not resource_id then
        return nil, "Invalid KhulnaSoft resource URL: " .. url
    end
