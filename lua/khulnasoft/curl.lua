...
function M.request(endpoint, req)
    ...
    req.headers =
        vim.tbl_extend("force", req.headers, { Accept = "application/json" })

    local auth = req.auth
        or require("khulnasoft.authentication").default_resolver():resolve()
    if auth then
        local token
...
function M.enabled()
    return not vim.g.khulnasoft_api_provider_curl_disabled
end
